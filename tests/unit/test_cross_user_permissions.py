...
def _make_workflow(**overrides):
    base = {
        "workflow_id": "wf-1395",
        # user_id None keeps _get_gh off the real users table (the mocked
        # GitHubOps ignores system_account; hermetic under _isolated_unit_db).
        "user_id": None,
        "title": "gh issue 1395",
...
def _make_orchestrator(wf):
    ...
    with (
        ...
        o._resolve_system_account = MagicMock(return_value=None)
    return o, mock_repo
...
class TestGetGhRerunFallback:
    ...
    def _wf(self, **overrides):
        base = {
            "workflow_id": "wf-1395",
            # user_id None keeps _get_gh off the real users table (the
            # class-patched GitHubOps ignores system_account).
            "user_id": None,
            "project_path": "/home/rhuang/open-ace",
