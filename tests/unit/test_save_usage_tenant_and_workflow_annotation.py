...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1760)]
...
def test_update_agent_sessions_stats_writes_per_session_model(tmp_path, monkeypatch):
    ...
    _cfg_path = Path(__file__).resolve().parents[2] / "scripts" / "shared" / "config.py"
    ...
    monkeypatch.setattr(shared_db, "_db_url_cache", f"sqlite:///{db_path}")
    monkeypatch.setattr(shared_db, "DB_DIR", str(tmp_path))
    monkeypatch.setattr(shared_db, "DB_PATH", str(db_path))
    monkeypatch.setattr(shared_db, "is_postgresql", lambda: False)
