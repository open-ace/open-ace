...
pytestmark = [pytest.mark.regression, pytest.mark.issue(673)]
...
@pytest.fixture
def seeded_sessions(db, user_id):
    ...
    for session_id, status, session_type, title in sessions_data:
        db.execute(
            ...
            """,
            (session_id, user_id, status, session_type, title, now, now),
        )
...
def test_status_filter(db, user_id, seeded_sessions):
    """测试 status 过滤参数"""
    # 查询 active 状态的会话
    ...
    active_count = len([sid for sid in seeded_sessions if "active" in sid])
    assert (
...
def test_session_type_filter(db, user_id, seeded_sessions):
    ...
    chat_count = len([sid for sid in seeded_sessions if "chat" in sid])
...
def test_combined_filter(db, user_id, seeded_sessions):
    ...
    active_chat_count = len([sid for sid in seeded_sessions if "active_chat" in sid])
    assert (
...
def test_invalid_status_filter(db, user_id, seeded_sessions):
    """测试无效 status 参数（应被忽略）"""
    # 使用无效的 status 值
    sessions = db.fetch_all(
        """
...
def test_invalid_session_type_filter(db, user_id, seeded_sessions):
...
def test_api_endpoint_integration(client, db, user_id, seeded_sessions):
    """
    ...
    """
    # 测试 status 过滤
    resp = _authed_get(client, "/api/workspace/sessions?status=active", user_id)
    ...
    # 测试 session_type 过滤
    resp = _authed_get(client, "/api/workspace/sessions?session_type=chat", user_id)
    assert resp.status_code == 200, f"API 返回 {resp.status_code}"
    data = resp.get_json()
    assert data.get("success") is True
    sessions = data.get("data", {}).get("sessions", [])
    ...
    # 测试组合过滤
    resp = _authed_get(client, "/api/workspace/sessions?status=active&session_type=chat", user_id)
    assert resp.status_code == 200, f"API 返回 {resp.status_code}"
    data = resp.get_json()
    assert data.get("success") is True
    sessions = data.get("data", {}).get("sessions", [])
    ...
    resp = _authed_get(client, "/api/workspace/sessions?status=invalid_status", user_id)
    assert resp.status_code == 200, f"API 返回 {resp.status_code}"
    data = resp.get_json()
    assert data.get("success") is True
    sessions = data.get("data", {}).get("sessions", [])
