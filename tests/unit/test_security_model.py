...
# Modules under test hold their own ``from ... import is_postgresql`` binding,
# which the ``db_mod`` patch below does not reach. Importing them eagerly lets
# us patch each module's bound reference so the SQLite shim is applied
# regardless of whether an earlier test already imported them, and regardless
# of whether the host's config defaults to PostgreSQL.
...
def _patch_db_compat():
    """Force every component in the stack into SQLite mode.

    ``is_postgresql`` is module-level bound in several modules via
    ``from app.repositories.database import is_postgresql``. Patching it only on
    ``db_mod`` leaves those bound references pointing at the real function, so
    on a host whose config selects PostgreSQL the proxy/manager keep emitting
    ``%s`` placeholders against a SQLite DB. Patch every module that holds its
    own binding so each test is independent of import order.
    """
    with (
...
class TestPasswordHashing:
    ...
    hashing cost (a direct ``bcrypt.gensalt`` assertion would not).
    """

    def test_hash_password_uses_bcrypt_12_rounds(self):
        from app.routes.auth import hash_password

        hashed = hash_password("my-secret-password")
        # bcrypt format: $2b$<cost>$... — the project must hash at 12 rounds.
