...
pytestmark = [pytest.mark.regression, pytest.mark.issue(189)]


# Import remote-agent's executor (and the cli_adapters submodules its methods
# lazily import) while remote-agent is briefly on sys.path, then remove the
# entry: a leaked path shadows bare ``import config`` for later tests in the
# same worker (tests/unit/test_db.py). The cached namespaced modules stay
# importable from sys.modules without the path.
_REMOTE_AGENT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "remote-agent")
)
sys.path.insert(0, _REMOTE_AGENT_DIR)
try:
    import cli_adapters.usage_parser  # noqa: F401  (executor imports it lazily)
...
class TestSessionProcessInterrupt(unittest.TestCase):
    ...
    def _make_session(self, running=True, pid=12345):
        SessionProcess = executor_mod.SessionProcess
...
class TestProcessExecutorInterruptSession(unittest.TestCase):
    ...
    def _make_executor(self, sessions=None):
        ProcessExecutor = executor_mod.ProcessExecutor
...
class TestAgentAbortRequest(unittest.TestCase):
    ...
    def test_abort_request_dispatches_to_interrupt_session(self, MockExecutor):
        from tests.unit._helpers_1828 import load_agent_module

        RemoteAgent = load_agent_module().RemoteAgent
