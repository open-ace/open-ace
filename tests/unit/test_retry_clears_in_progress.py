...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1897)]
...
class TestRetryClearsInProgress:
    ...
    def test_clear_in_progress_safe_on_missing_workflow(self):
        ...
        sched = AutonomousScheduler()
        # No workflow_id in any set — a no-op that returns None without crash
        with patch("app.routes.autonomous._get_repo"):
            assert sched.clear_in_progress("nonexistent-wf") is None
