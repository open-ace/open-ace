...
Migrated from tests/issues/352/test_tool_limit_hints.py (sys.path bootstrap
re-scoped, resolved and guarded, for the tests/unit depth).
"""
...
pytestmark = [pytest.mark.regression, pytest.mark.issue(352)]

BASE_URL = os.environ.get("BASE_URL", "http://localhost:19888")

# Add remote-agent directory to path (idempotent; resolved so the guard
# compares canonical strings).
remote_agent_dir = (Path(__file__).resolve().parents[2] / "remote-agent").resolve()
