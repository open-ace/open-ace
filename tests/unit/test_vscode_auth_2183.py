...
pytestmark = [pytest.mark.regression, pytest.mark.issue(2183)]
...
def _make_app(mgr):
    ...
    The module globals the blueprint resolves at request time
    (``remote_agent_manager._agent_manager`` and the token loaders) are
    patched for the lifetime of the context manager and restored on exit —
    the pre-migration version assigned them directly and never restored,
    leaking a MagicMock manager and a stub token loader into every later
    test in the same process. ``remote._load_user_from_token`` does not exist
    on the production module, hence ``create=True``.
    """
    ...
    with (
        patch.object(ram_mod, "_agent_manager", mgr),
        patch.object(auth_dec, "_load_user_from_token", _mock_load_user),
        patch.object(remote_mod, "_load_user_from_token", _mock_load_user, create=True),
    ):
        yield app
...
class TestVSCodeTokenAuthentication(unittest.TestCase):
    ...
    def test_valid_token_allows_access(self):
        ...
        with patch.object(vs_mod, "vscode_info_store", test_store), _make_app(mgr) as app:
