...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1771), pytest.mark.security]
...
class TestWebhookAsyncDispatch:
    def test_create_alert_does_not_block_on_slow_webhook(self):
        ...
        with (
            ...
            # The #2063 dispatch worker claims a DB-backed cooldown row (and
            # would re-read central settings) before POSTing. Stub the claim
            # with its documented "store unavailable" outcome so the worker
            # takes the fail-open single-attempt path straight to the mocked
            # Session.post — no DB, no extra latency.
            patch.object(
                AlertNotifier,
            ...
            # create_alert has already returned (elapsed measured above); hold
            # the patches active until the worker has actually reached the
            # mocked Session.post, so the delivery start is deterministic
            # regardless of thread scheduling. This wait does not affect the
