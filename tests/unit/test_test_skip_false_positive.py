...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1277)]
...
def _make_orchestrator(wf_data, milestones=None):
    ...
    with (
        ...
        # System-account resolution reads the users table; keep the test on the
        # mocked repo by resolving to the workflow's own user (None is valid).
...
class TestSchemaIncludesRetryColumns:
    ...
    def test_update_skip_retries_succeeds_on_fresh_sqlite(self, tmp_path):
        ...
        def _fetch_one(query, params=()):
            c = sqlite3.connect(db_path)
            # Row-based access mirrors Database.fetch_one's dict-style rows.
        ...
        # This must not raise OperationalError, must report success (the
        # dict|None contract: None only on optimistic-lock failure), and the
        # column must round-trip through a fresh read.
        updated = repo.update_workflow("wf-ddl", {"skip_retries": 1})
        assert updated is not None
        assert updated["skip_retries"] == 1
