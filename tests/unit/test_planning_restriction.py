...
pytestmark = [pytest.mark.regression, pytest.mark.issue(761)]
...
class TestClaudeCodeAdapterAllowedTools:
    ...
    def _add_remote_agent_to_path(self):
        ...
        ra = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "..", "remote-agent"))
        # Scoped insert (tests/unit/_helpers_1828.py hazard): the entry is
        # removed again after the test, and any ``config`` the adapter load
        # cached is rolled back — pytest imports every module at collection
        # time, so a leaked remote-agent entry would sit ahead of the
        # scripts/shared insert tests/unit/test_db.py relies on and shadow
        # its bare ``import config``.
