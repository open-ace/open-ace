...
pytestmark = [pytest.mark.security, pytest.mark.regression, pytest.mark.issue(1773)]
...
class FakeFeishuOrgSyncService(FeishuOrgSyncService):
    ...
    def _fetch_directory_snapshot(self, token: str, warnings=None, errors=None):
...
def test_scheduler_hook_logs_exception_with_traceback(sync_env, caplog, monkeypatch):
    ...
    monkeypatch.setattr(feishu_module, "FeishuOrgSyncService", BoomService)
