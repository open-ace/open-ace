"""Terminal WS bridge + backend URL tests for the remote agent manager (Issue #555).

The per-user remote-agent terminal WebSocket is bridged through the main
backend route (``/api/remote/terminal/<id>/ws``) with a per-terminal browser
token, and ``get_backend_url`` prefers the configured external_url over the
request host. Migrated from tests/issues/555/test_terminal_main_ws_bridge.py
(the CONFIG_DIR monkeypatch keeps the config reads hermetic).
"""
...
pytestmark = [pytest.mark.regression, pytest.mark.issue(555)]
