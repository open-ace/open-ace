...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1525)]
...
def _make_workflow(**overrides):
    ...
    base = {
        ...
        # No user_id: _do_preparation resolves the owner's system_account
        # inline via UserRepository; the unit layer has no users table.
        "user_id": None,
        "title": "Test Workflow",
        "status": "pending",
...
def _make_orchestrator(wf_data):
    ...
    with (
        ...
        # Hermetic system-account resolution (ambient users table not present
        # in the unit layer's isolated sqlite; pattern from batch 7's 826).
