...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1776)]
...
def _exec_scoped(relative_path: str, module_name: str):
    """Exec a remote-agent module under a unique name, with both process-global
    side effects of the load scoped to the call.

    remote-agent is forced to the front of ``sys.path`` only while the module
    executes (its siblings ``cli_settings``/``config``/... are bare-name
    imports), and any ``config`` entry the exec caches is rolled back — a
    leaked remote-agent ``config`` shadows the repo-level top-level ``config``
    package for every later test in this worker (tests/unit/test_db.py imports
    the scripts-side one).
    """
    module_path = REPO_ROOT / "remote-agent" / relative_path
...
def load_cli_settings():
    return _exec_scoped("cli_settings.py", "cli_settings_1776")


def load_openace_cli():
    return _exec_scoped("openace_cli.py", "openace_cli_1776")
...
def test_write_codex_settings_skips_chmod_on_windows(tmp_path):
    ...
    module_path = REPO_ROOT / "remote-agent" / "cli_settings.py"
...
def load_agent_module():
    """``agent.py`` imports its sibling ``config`` as a bare name, so the load
    pops any cached ``config`` first (mirroring the pre-existing behavior) and
    restores the previous entry afterwards via ``_exec_scoped``."""
    return _exec_scoped("agent.py", "remote_agent_1776")
