...
# Scoped bootstrap (tests/unit/_helpers_1828.py hazard): remote-agent joins
# sys.path only while its sibling modules are being imported, and the
# ``config`` entry those imports cache is rolled back afterwards. A leaked
# remote-agent ``config`` would shadow the repo-level top-level ``config``
# package for every later test in the same worker (tests/unit/test_db.py
# imports the scripts-side one).
_saved_config = sys.modules.pop("config", None)
_path_added = str(AGENT_DIR) not in sys.path
if _path_added:
...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1892), pytest.mark.security]


def _load_agent_module(filename: str, name: str):
    """Load a remote-agent module under a unique, non-colliding name.

    Mirrors the scoped bootstrap above: the temporary remote-agent sys.path
    entry and the sibling ``config`` module the exec imports are rolled back
    once the module object exists, so the load leaves no trace in the process.
    """
