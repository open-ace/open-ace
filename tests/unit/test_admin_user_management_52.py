"""Unit tests for Issue 52: Management页面Users tab增加Linux Account功能.

覆盖（#2457 realignment 后语义不变）：
1. 数据库层面：system_account 字段（原 linux_account，随多用户工作区改造更名）
2. API 层面：更新用户接口支持 system_account，密码重置接口
3. 用户列表投影：配额脚本固定投影列 + system_account 经详情读取
...
Migrated from tests/issues/52/test_issue52.py by batch 16 (#2429); R3
conversion: the legacy script returned bools from every body (PytestReturn-
NotNoneWarning, pass/fail only via the __main__ runner) — bodies now end in
plain pytest asserts with each predicate preserved.
"""
