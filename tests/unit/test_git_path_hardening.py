...
pytestmark = [pytest.mark.regression, pytest.mark.issue(2021), pytest.mark.security]
...
def test_canonical_containment_accepts_nested_child(tmp_path):
    ...
    # Should not raise — success is the explicit None return.
    assert _assert_path_contained(str(nested), str(tmp_path), label="test") is None
...
def test_register_trusted_git_context_accepts_linked_worktree(tmp_path):
    ...
    # and git_dir is under common_dir (main_repo/.git). Success must pin the
    # trusted context for the realpath'd worktree.
    try:
        gh.register_trusted_git_context(
        ...
        pinned = GitHubOps._trusted_git_contexts[os.path.realpath(str(worktree))]
        assert pinned["git_dir"] == str(worktree_git_dir)
        assert pinned["git_identity"] == "dev:1"
    finally:
        GitHubOps._trusted_git_contexts.pop(os.path.realpath(str(worktree)), None)
...
def test_register_trusted_git_context_accepts_main_repo(tmp_path):
    ...
    try:
        ...
        pinned = GitHubOps._trusted_git_contexts[os.path.realpath(str(repo))]
        assert pinned["git_dir"] == str(repo / ".git")
        assert pinned["work_tree"] == os.path.realpath(str(repo))
    finally:
        GitHubOps._trusted_git_contexts.pop(os.path.realpath(str(repo)), None)
