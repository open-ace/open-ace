...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1138)]

# tests/unit/<file> → repo root → remote-agent. Guarded per-test inserts below
# (sibling pattern: only the tests that import remote-agent packages mutate
# sys.path; the dispatch tests need nothing — _run_local adds the dir itself).
_REMOTE_AGENT_DIR = str(Path(__file__).resolve().parents[2] / "remote-agent")


def _ensure_remote_agent_dir() -> None:
...
def test_claude_code_still_uses_local_session(runner):
    """claude-code must NOT use zcode or single-shot paths — it stays on the
    _LocalSession (stream-json) spawn path.

    The spawn is mocked at the sandbox-provider exec seam (and the executable
    lookup at shutil.which), so no real ``claude`` binary can ever run on a
    developer PATH: provider.exec raises OSError, which is exactly the
    "process failed to start" branch of the local path. Reaching that branch
    (rather than returning early or raising) is itself the routing assertion.
    """
    from app.modules.workspace.autonomous.agent_runner import AutonomousAgentRunner

    mock_provider = MagicMock()
    mock_provider.exec.side_effect = OSError("claude spawn blocked in test")
    runner._sandbox_provider = mock_provider

    with patch.object(
        AutonomousAgentRunner, "_build_agent_env", return_value={"PATH": "/usr/bin:/bin"}
    ):
        with patch(
    ...
    # Routing: claude-code stays on the local stream-json path.
    mock_zc.assert_not_called()
    mock_ss.assert_not_called()
    # The local spawn path was genuinely attempted (exec seam hit once)…
    mock_provider.exec.assert_called_once()
    # …and failed at the mocked spawn, not by falling into another route.
    assert result.success is False
    assert "Failed to start process" in result.error
    # The sandbox handle created before the failed spawn must be destroyed.
...
def test_zcode_session_start_failure_cleans_up(runner):
    ...
    with patch("app.modules.workspace.autonomous.agent_runner.subprocess.Popen") as mock_popen:
        ...
        # _run_zcode_appserver builds the spawn env through the #2019
        # env_security gate (_build_agent_env → DB-backed LLM-proxy minting),
        # which refuses to launch in tests. These tests assert session-failure
        # CLEANUP, not credential handling, so stub the env builder (canonical
        # pattern: test_zcode_workflow_session_persistence.py /
        # test_cross_user_permissions.py::TestSingleShotUserIdPropagation).
        with patch.object(
            AutonomousAgentRunner, "_build_agent_env", return_value={"PATH": "/usr/bin:/bin"}
        ):
            with patch.object(
                zcode_app_server, "ZCodeAppServerSession", return_value=mock_zc_instance
            ):
