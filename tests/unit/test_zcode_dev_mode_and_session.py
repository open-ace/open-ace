...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1140)]

# Module-level guarded insert so every test in this file can import the
# remote-agent packages (cli_adapters / zcode_app_server) regardless of test
# order — single-file and -k-filtered runs must not depend on another test
# having populated sys.path first.
...
def _run_zcode_with_mock(permission_mode, fake_build):
    ...
    # _run_zcode_appserver now builds the agent env through the #2019
    # env_security gate (_build_agent_env → LLM-proxy minting), which needs a
    # DB-backed proxy service and refuses to launch without it. These tests
    # assert MODE RESOLUTION, not credential handling, so stub the env builder
    # (an ordinary staticmethod; patch.object restores the descriptor) per the
    # canonical pattern in test_zcode_workflow_session_persistence.py /
