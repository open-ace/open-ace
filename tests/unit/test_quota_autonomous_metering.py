...
class TestOrchestratorRefreshesOwnerQuotaUsage:
    """The orchestrator must refresh user_daily_stats after an autonomous step,
    so check_quota's fast path sees the spend without waiting for the periodic
    aggregator (#1269 P0-2)."""

    def test_refresh_owner_quota_usage_called_on_step_completion(self):
        ...
        with (
            ...
            mock_repo = MagicMock()
            mock_repo.get_workflow.return_value = {"user_id": 7}
            mock_repo.list_milestones.return_value = []
            mock_repo_cls.return_value = mock_repo

            from app.modules.workspace.autonomous.orchestrator import AutonomousOrchestrator

            orch = AutonomousOrchestrator("wf-1")
            orch.repo = mock_repo
            orch.emitter = MagicMock()

            with patch("app.services.user_stats_aggregator.get_aggregator") as mock_get_agg:
                ...
                orch._write_phase_usage("ms-1", result)

                # aggregate_today(owner_id) must be invoked to refresh the
                # owner's today roll-up so the gate's fast path is current.
                mock_get_agg.return_value.aggregate_today.assert_called_once_with(7)
