...
pytestmark = [pytest.mark.security, pytest.mark.regression, pytest.mark.issue(1810)]


def load_fetch_openclaw():
    module_path = Path(__file__).resolve().parents[2] / "scripts" / "fetch_openclaw.py"
...
class FakeDingTalkOrgSyncService(DingTalkOrgSyncService):
    ...
    def _fetch_directory_snapshot(self, token, root_department_id, **kwargs):
        # Complete snapshot: departed-user cleanup may proceed (the behavior
        # under test). Production contract is a 3-tuple (#2389 snapshot flag).
...
def test_deactivate_departed_users_isolates_by_tenant(sync_env):
    ...
    # --- Tenant A re-syncs with alice departed (carol replaces her). ---
    # #3020 made empty snapshots skip departed-user cleanup entirely (mass-
    # deactivation guard), so the seen set must be non-empty for cleanup to
    # run. Alice is absent from this snapshot; bob (tenant B) is simply not in
    # tenant A's directory.
