...
pytestmark = [pytest.mark.regression, pytest.mark.issue(36)]
        ...
        try:
            ...
            if message_count > 0:
                ...
                if await user_messages.count() > 0:
                    ...
                    if await sender_element.count() > 0:
                        ...
                        assert ".local" not in displayed_sender.lower() and (
                            displayed_sender.count("-") <= 1
                        ), f"sender name not simplified: '{displayed_sender}'"
                    ...
                    if await host_element.count() > 0:
                        ...
                        assert (
                            ".local" not in displayed_host.lower()
                        ), f"host name not simplified: '{displayed_host}'"
                else:
            ...
            if await sender_filter.count() > 0:
                ...
                for i in range(1, min(4, option_count)):
                    ...
                    assert (
                        ".local" not in option_text.lower()
                    ), f"sender option not simplified: '{option_text}'"
            else:
            ...
            if await host_filter.count() > 0:
                ...
                for i in range(1, min(4, option_count)):
                    ...
                    assert (
                        ".local" not in option_text.lower()
                    ), f"host option not simplified: '{option_text}'"
            else:
