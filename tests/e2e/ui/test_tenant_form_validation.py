...
pytestmark = [pytest.mark.regression, pytest.mark.issue(221)]
        ...
        try:
            ...
            if await save_btn.count() > 0:
                ...
                if is_disabled:
                    ...
                    assert (
                        is_disabled
                    ), "Save button should be disabled when tenant name is empty (issue #221)"
            else:
            ...
            if not is_disabled_now:
                ...
                assert (
                    not is_disabled_now
                ), "Save button should be enabled after filling the tenant name (issue #221)"
