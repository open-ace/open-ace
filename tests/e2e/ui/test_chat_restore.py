...
pytestmark = [pytest.mark.regression, pytest.mark.issue(70)]
...
def test_chat_restore():
    ...
    with sync_playwright() as p:
                ...
                try:
                    ...
                    if "token=" in frame_url:
                        try:
                            ...
                            # 检查是否有消息元素
                            message_elements = frame.locator(
        ...
        assert failed == 0, (
            f"{failed} chat-restore check(s) failed: "
            f"{[name for name, result in test_results if not result]}"
        )
        return failed == 0
