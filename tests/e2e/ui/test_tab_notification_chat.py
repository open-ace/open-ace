...
def select_project_in_iframe(chat_frame, page, preferred_project="open-ace"):
    ...
    project_rows = chat_frame.locator(
        "div[class*='rounded-lg'][class*='p-4'], div[class*='cursor-pointer']"
    )
            ...
            if tab1_notification:
                ...
                if tab1_notification["has_badge"]:
                    ...
                    if tab1_notification["badge_content"] == "●":
                        ...
                        print(
                            f"    ✗ 徽章内容是 '{tab1_notification['badge_content']}' - 应该是 ●!"
                        )
            ...
            for i in range(tabs.count()):
                ...
                if notification and notification["has_badge"]:
                    if notification["badge_color"] in ["red", "yellow"]:
                        ...
                        print(
                            f"    ✓ Tab {i+1} 徽章颜色正确: {notification['badge_color'] or 'blue'}"
                        )
...
# --- issue #71 real-chat variant (merged from tests/issues/71) -------------
# The wave above pins the two-tab badge flow; this variant drives a single
# real chat and checks the bell/badge presentation on the tabs.
...
def test_tab_notification_chat_real_chat():
    """测试真实的聊天场景下的 tab 通知（issue #71 后续波次合并）"""
    _skip_if_no_server()
        ...
        try:
            ...
            if tab_count >= 2:
                ...
                if badge.count() > 0:
                    ...
                    assert (
                        "bg-info" in badge_classes
                    ), f"waiting badge should be blue (bg-info), got: {badge_classes}"
