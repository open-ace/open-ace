...
pytestmark = [pytest.mark.regression, pytest.mark.issue(65)]


# The lane runs the server on an ephemeral port and exports it as BASE_URL.
...
async def test_workspace_state_restore():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.goto(f"{BASE_URL}/work/usage")
            await page.wait_for_load_state("networkidle", timeout=30000)
            ...
            ), f"workspace tabs lost on navigation: {tab_count_before} -> {tab_count_after}"
