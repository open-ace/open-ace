...
pytestmark = [pytest.mark.regression, pytest.mark.issue(99)]
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                error_alert_visible = error_alert.is_visible(timeout=1000)
                if error_alert_visible:
                    error_text = error_alert.text_content()
                    print(f"\n❌ UI Error: {error_text}")
            except PlaywrightError:
                # Alert locator unavailable mid-navigation — treat as no alert
                pass
            assert not error_alert_visible, "Add Prompt dialog rendered an error alert"
