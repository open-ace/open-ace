...
Run:
  python tests/e2e/remote/e2e_codex_real_session.py
"""
...
def test_codex_session_file_content():
    ...
    assert line_count > 0, f"session file {codex_session_file} is empty"
    assert event_count > 0, "session file contains no message events"
...
def test_codex_quota_tracking():
    ...
    if row["cnt"] == 0:
        print("    SKIP: no codex sessions recorded yet, nothing to verify")
        return
    assert (row["tokens"] or 0) > 0, "codex sessions exist but total tokens is zero"
