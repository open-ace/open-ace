...
pytestmark = [pytest.mark.regression, pytest.mark.issue(165)]
...
def test_b3_permission_permanent_ui():
    ...
    assert api_agent_permission_request(
        control_request, sid=chat_sid
    ), "permission request was not accepted by the API"
    pause(3)
...
def test_b6_error_i18n():
    ...
    try:
        ...
        pass
    zh_text = page_zh.locator("body").text_content() or ""
    assert zh_text.strip(), "ChatPage error state rendered no content (zh locale)"
    ...
    try:
        ...
        pass
    en_text = page_en.locator("body").text_content() or ""
    assert en_text.strip(), "ChatPage error state rendered no content (en locale)"
        ...
        try:
            ...
            for test_fn in [
                ...
                try:
                    ...
                    Exception
                ) as e:  # allow-swallow: collect errors per test; runner exits nonzero if any failed as e:
