...
def test_webui_quota_check_valid_token():
    ...
    data = response.json()
    assert "can_use" in data, f"response missing 'can_use' field: {data}"
...
def test_webui_quota_check_invalid_token():
    ...
    ), f"invalid token should be rejected with 401, got {response.status_code}: {response.text}"
...
def test_webui_quota_check_over_quota():
    ...
    for period in ["daily", "monthly"]:
        for metric in ["tokens", "requests"]:
            assert metric in data[period], f"missing '{metric}' in {period}"
            metric_data = data[period][metric]
            assert (
                "used" in metric_data and "over_quota" in metric_data
            ), f"missing required fields in {period}.{metric}: {metric_data}"
...
def test_webui_quota_check_missing_token():
    ...
    assert (
        response.status_code == 401
    ), f"missing token should be rejected with 401, got {response.status_code}: {response.text}"
