...
pytestmark = [pytest.mark.regression, pytest.mark.issue(52)]
...
SCREENSHOT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(skill_dir))), "screenshots", "issues", "52"
)
...
async def test_issue52():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            screenshots.append(await take_screenshot(page, "01_login_page.png"))
            ...
            await page.wait_for_load_state("networkidle")
            time.sleep(2)
            screenshots.append(await take_screenshot(page, "02_after_login.png"))
            ...
            await page.wait_for_load_state("networkidle")
            time.sleep(1)
            screenshots.append(await take_screenshot(page, "03_management_page.png"))
            ...
            table_headers = await page.locator("#users-content table thead th").all_inner_texts()
            print(f"  Table headers: {table_headers}")

            assert (
                "Linux Account" in table_headers
            ), "Linux Account column not found in table headers"
            print("  ✓ Linux Account column found in table headers")
            screenshots.append(await take_screenshot(page, "04_users_table.png"))
            ...
            if len(rows) > 0 and "Loading" not in await rows[0].inner_text():
                ...
                await expect(edit_btn).to_be_visible()
                print("  ✓ Edit button found")

                await expect(password_btn).to_be_visible()
                print("  ✓ Password reset button found")

                await expect(delete_btn).to_be_visible()
                print("  ✓ Delete button found")

                screenshots.append(await take_screenshot(page, "05_action_buttons.png"))
            else:
            ...
            if len(rows) > 0 and "Loading" not in await rows[0].inner_text():
                ...
                await expect(linux_account_input).to_be_visible()
                print("  ✓ Linux Account input field found in modal")

                screenshots.append(await take_screenshot(page, "06_edit_modal.png"))

                # Close modal
                await page.click("#editUserModal .btn-close")
                time.sleep(0.5)
