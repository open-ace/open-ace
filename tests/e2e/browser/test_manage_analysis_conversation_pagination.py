...
Run (normal):
  HEADLESS=true  python tests/e2e/browser/test_manage_analysis_conversation_pagination.py
  HEADLESS=false python tests/e2e/browser/test_manage_analysis_conversation_pagination.py
...
built dist):
  WEB_BASE=http://localhost:3000 API_BASE=http://localhost:19888 HEADLESS=true \\
    python tests/e2e/browser/test_manage_analysis_conversation_pagination.py
"""
...
pytestmark = [pytest.mark.regression, pytest.mark.issue(824)]
...
def check(desc, condition, detail=""):
    """Raise-ified historical check (#824 exodus): every check is a hard assert."""
    assert condition, f"{desc} {detail}"
...
def test_pagination():
    ...
    assert token, f"login did not yield a session token (http={login_code})"
    ...
    if total_pages <= 1:
        # Data precondition, not a product regression: pagination needs a
        # multi-page dataset to exercise.
        pytest.skip(
            f"not enough data to exercise pagination (http={code}, "
    ...
    with sync_playwright() as p:
        ...
        print("\n" + "=" * 60)
        print("RESULTS: all pagination checks passed")
        print("=" * 60)
        browser.close()
