...
def test_login_success():
    """测试正确凭据登录成功"""
    import tests.e2e.browser.test_helpers as helpers
        ...
        try:
            ...
            try:
                ...
                user_menu = page.locator(
                    ".dropdown:has(> button.dropdown-toggle > .bi-person-circle)"
                )
                user_menu.wait_for(state="visible", timeout=15000)
                user_menu.locator("button.dropdown-toggle").click()

                # Wait for dropdown menu to be visible (not just a fixed timeout)
                user_menu.locator(".dropdown-menu").wait_for(state="visible", timeout=3000)
