...
SCREENSHOT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'screenshots', 'issues', '92')
...
def test_logout_position():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            test_results.append(("Language Selector Visible", "PASS", "Language selector is visible"))
            ...
            if logout_box and lang_box:
                ...
                if logout_box['y'] < lang_box['y']:
                    test_results.append(("Logout Above Language", "PASS", 
                        f"Logout Y: {logout_box['y']:.0f}, Lang Y: {lang_box['y']:.0f}"))
                else:
                    test_results.append(("Logout Above Language", "FAIL", 
                        f"Logout Y: {logout_box['y']:.0f} should be < Lang Y: {lang_box['y']:.0f}"))
            else:
                test_results.append(("Logout Above Language", "FAIL", "Could not get bounding boxes"))
            ...
            if logout_pos != -1 and lang_pos != -1 and logout_pos < lang_pos:
                test_results.append(("DOM Order Correct", "PASS", 
                    f"Logout at position {logout_pos}, Lang at position {lang_pos}"))
            else:
                test_results.append(("DOM Order Correct", "FAIL", 
                    f"Logout at {logout_pos}, Lang at {lang_pos}"))
