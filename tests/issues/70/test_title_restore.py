...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print("\n[6] 检查当前 tab 标题...")
            tab_title = page.locator(
                "div[class*='tab'][class*='active'] span, div[data-active] span"
            ).first.text_content()
            ...
            tab_element = page.locator(
                "div[class*='tab'][class*='active'], div[data-active='true']"
            ).first
            ...
            if "open-ace-store" in storage_data:
                ...
                for i, tab in enumerate(tabs):
                    print(
                        f"    Tab {i+1}: title='{tab.get('title')}', sessionId={tab.get('sessionId', 'N/A')[:15]}..."
                    )
