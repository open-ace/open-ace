...
4. X-Session-Id header still takes precedence
5. Webui sessions are filtered from lookup
6. Exception handling: fallback to webui aggregate session on error
"""
...
class TestExceptionHandling:
    ...
    @patch(_SESSION_MGR_PATH)
    @patch(_PROXY_PATH)
    def test_fallback_to_webui_on_exception(
        self,
        mock_get_proxy,
        mock_session_mgr,
        mock_quota_cls,
        ...
        """Should fall back to webui aggregate session on exception."""
        mock_proxy = MagicMock()
        mock_proxy.validate_proxy_token.return_value = _mock_proxy_token()
        mock_proxy.get_tool_model_pool.return_value = {
            "models": [{"id": "qwen3"}],
        ...
        # Simulate exception during get_active_sessions
        mock_sm = MagicMock()
        mock_sm.get_active_sessions.side_effect = Exception("Database error")
        mock_session_mgr.return_value = mock_sm
        ...
        # Should still succeed using webui aggregate session
        assert resp.status_code == 200