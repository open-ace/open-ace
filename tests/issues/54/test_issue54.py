...
class TestIssue54:
    ...
    def test_add_user_modal_fields(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                print("\n[Step 3] Navigate to Management page")
                management_nav = page.locator('#nav-management:visible')
                ...
                print("\n[Step 6] Verify Linux Account field exists")
                linux_account_input = page.locator('#add-linux-account')
                expect(linux_account_input).to_be_visible()
                print("  ✓ Linux Account field is visible")

                # Verify Linux Account label
                linux_account_label = page.locator('label[for="add-linux-account"]')
                ...
                linux_account_hint = page.locator('#add-linux-account + small.form-text')
                ...
                print("\n[Step 7] Verify Confirm Password field exists")
                confirm_password_input = page.locator('#add-confirm-password')
                expect(confirm_password_input).to_be_visible()
                print("  ✓ Confirm Password field is visible")

                # Verify Confirm Password label
                confirm_password_label = page.locator('label[for="add-confirm-password"]')
                ...
                page.locator('#add-username').click()
                page.wait_for_timeout(300)

                # Check if error message is displayed
                password_match_msg = page.locator('#password-match-msg')
