...
async def test_add_user_save():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 使用 label 定位输入框
            await page.locator('label:has-text("Username") + input, input[placeholder*="username"]').first.fill('testuser79')
            await page.locator('label:has-text("Email") + input, input[type="email"]').first.fill('test79@test.com')
            ...
            # 填写有效数据
            await page.locator('label:has-text("Username") + input, input[placeholder*="username"]').first.fill('testuser79_valid')
            await page.locator('label:has-text("Email") + input, input[type="email"]').first.fill('test79_valid@test.com')
