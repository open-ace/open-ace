...
async def test_issue32():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 先截图看看当前状态
            await page.screenshot(path=f'{SCREENSHOT_DIR}/issue32_before_tab_click_{timestamp}.png', full_page=True)
            ...
            # 截图看看点击后的状态
            await page.screenshot(path=f'{SCREENSHOT_DIR}/issue32_after_tab_click_{timestamp}.png', full_page=True)
            
            # 等待表格初始化
            try:
                await page.wait_for_selector('#conversation-history-table .tabulator-row', timeout=15000)
            ...
            # 截图
            await page.screenshot(path=f'{SCREENSHOT_DIR}/issue32_timeline_modal_{timestamp}.png', full_page=True)
            ...
            # 最终截图
            await page.screenshot(path=f'{SCREENSHOT_DIR}/issue32_timeline_detail_{timestamp}.png', full_page=True)
            print(f"   ✓ 详细截图已保存")

        except Exception as e:
            print(f"   ✗ 测试出错: {str(e)}")
            results.append(("测试执行", False, str(e)))
            await page.screenshot(path=f'{SCREENSHOT_DIR}/issue32_error_{timestamp}.png', full_page=True)
