...
async def test_timeline_no_toolresult():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 截图
            await page.screenshot(path=f'{SCREENSHOT_DIR}/timeline_modal_{timestamp}.png', full_page=True)
            ...
            invalid_roles = [role for role in role_labels if role not in ['User', '用户', 'Assistant', 'AI 助手', 'AI']]
            ...
            # 最终截图
            await page.screenshot(path=f'{SCREENSHOT_DIR}/timeline_final_{timestamp}.png', full_page=True)
            ...
            # 异常时截图
            try:
                await page.screenshot(path=f'{SCREENSHOT_DIR}/error_{timestamp}.png', full_page=True)
            except:
                pass
...
async def test_api_timeline_no_toolresult():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(f"{BASE_URL}/api/conversation-history?start={utils.get_days_ago(7)}&end={utils.get_today()}&page=1&limit=20") as resp:
            if resp.status != 200:
        ...
        async with session.get(f"{BASE_URL}/api/conversation-timeline/{session_id}") as resp:
            ...
            if toolresult_count > 0:
                ...
                results.append(("ToolResult 过滤验证", False, f"发现 {toolresult_count} 条 toolResult"))
            else:
