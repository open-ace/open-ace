        ...
        try:
            ...
            if button_count > 0:
                ...
                if is_disabled:
                    ...
                    pointer_events = await view_all_button.last.evaluate('el => window.getComputedStyle(el).pointerEvents')
                ...
                if box:
                    print(f"  按钮位置：x={box['x']:.1f}, y={box['y']:.1f}, width={box['width']:.1f}, height={box['height']:.1f}")
                    
                    if box['width'] > 0 and box['height'] > 0:
                        print("  ✓ 按钮有有效的尺寸")
                        results.append(("按钮尺寸有效", True, f"{box['width']:.1f}x{box['height']:.1f}"))
                    else:
                        print("  ✗ 按钮尺寸无效（宽度或高度为0）")
                        results.append(("按钮尺寸有效", False, f"{box['width']:.1f}x{box['height']:.1f}"))
                else:
                    print("  ✗ 无法获取按钮位置")
