    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if session_list_count > 0:
                ...
                overflow_style = await session_list.evaluate('el => window.getComputedStyle(el).overflow')
                ...
                if session_groups_count > 0:
                    groups_overflow = await session_groups.evaluate('el => window.getComputedStyle(el).overflowY')
                    ...
                    if groups_overflow in ['auto', 'scroll']:
                        ...
                        results.append(("session-groups overflow-y 设置正确", True, groups_overflow))
                    else:
                ...
                main_scroll = await work_main.evaluate('el => ({ scrollTop: el.scrollTop, scrollHeight: el.scrollHeight, clientHeight: el.clientHeight })')
                print(f"  work-main scroll: scrollTop={main_scroll['scrollTop']}, scrollHeight={main_scroll['scrollHeight']}, clientHeight={main_scroll['clientHeight']}")
                results.append(("主内容区域滚动检查", True, f"scrollTop={main_scroll['scrollTop']}"))
            ...
            page_scroll = await page.evaluate('() => ({ scrollTop: document.documentElement.scrollTop || document.body.scrollTop, scrollHeight: document.documentElement.scrollHeight })')
            print(f"  页面滚动: scrollTop={page_scroll['scrollTop']}, scrollHeight={page_scroll['scrollHeight']}")
            ...
            layout_overflow = await work_layout.evaluate('el => window.getComputedStyle(el).overflow')
            ...
            if layout_height == '100vh' or layout_overflow == 'hidden':
                ...
                print(f"  ! work-layout height: {layout_height}, overflow: {layout_overflow}")
                results.append(("work-layout 高度设置正确", True, f"height={layout_height}, overflow={layout_overflow}"))
