    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if labels_match:
                ...
                results.append(("Tab 标签正确", False, f"期望 {expected_labels}, 实际 {tab_labels}"))
            ...
            for i in range(min(tabs_count, 3)):
                ...
                if box:
                    ...
                    print(f"    Tab {i+1} ({tab_labels[i] if i < len(tab_labels) else 'Unknown'}): "
            ...
            if len(tab_positions) >= 2:
                ...
                for tab in tab_positions[1:]:
                    ...
                    if y_diff > 5:  # 5px tolerance
                        same_row = False
                        print(f"  ✗ Tab '{tab['label']}' 的 Y 位置 ({tab['y']:.1f}) 与第一个 Tab ({first_y:.1f}) 差异过大")
                        break
            ...
            nav_tabs_display = await nav_tabs_element.evaluate('el => window.getComputedStyle(el).display')
            nav_tabs_flex_direction = await nav_tabs_element.evaluate('el => window.getComputedStyle(el).flexDirection')
