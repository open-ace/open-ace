...
def test_refresh_functionality():
        ...
        try:
            ...
            if scheduler_response.get('success'):
                ...
                if scheduler.get('enabled') and scheduler.get('running'):
                    results.append(('Backend Scheduler', 'PASS',
                                   f'Running with interval {scheduler.get("interval")}s'))
                else:
                    results.append(('Backend Scheduler', 'FAIL',
            ...
            refresh_btn = page.locator('header button.btn-outline-primary:has-text("刷新"), header button.btn-outline-primary:has-text("Refresh")')
            ...
            if switch_count > 0:
                ...
                results.append(('Auto-refresh Toggle', 'PASS', f'Found, currently {"ON" if is_checked else "OFF"}'))
            else:
            ...
            if switch_count > 0:
                ...
                if not auto_refresh_switch.is_checked():
                    ...
                    if is_checked:
                        results.append(('Auto-refresh Toggle Action', 'PASS', 'Successfully toggled ON'))
                    else:
                        results.append(('Auto-refresh Toggle Action', 'FAIL', 'Could not toggle ON'))
                else:
                ...
                screenshots.append(('screenshots/refresh_test_03_auto_refresh_on.png', 'Auto-refresh ON'))
            else:
                results.append(('Auto-refresh Toggle Action', 'SKIP', 'No auto-refresh switch to test'))
    ...
    print("-" * 60)
    print(f"Total: {len(results)}, Passed: {passed}, Failed: {failed}, Skipped: {skipped}, Warnings: {warned}, Errors: {errors}")
    print("=" * 60)
