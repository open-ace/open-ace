        ...
        try:
            ...
            for req in requests_log:
                ...
                if "/api/" in url:
                    ...
                    ext in url for ext in ["static", ".js", ".css", ".woff", ".woff2", ".html"]
                ):
            ...
            print("  + React Render:     ~1.0s (estimated)")
            print("  ─────────────────────────────")
            ...
            if static_requests:
                sorted_static = sorted(
                    static_requests,
                    key=lambda r: r["end"] - r["start"] if r["end"] else 0,
                    reverse=True,
                )
            ...
            if static_times and api_times:
                ...
                print("  (largest resource + API requests)")

                # Overhead
                overhead = overall_time - critical_path
                print(f"  Other overhead:         {overhead:.3f}s")
                print("  (includes: parsing, rendering, React hydration)")
            ...
            print(
                f"  - Static resources:      ~{sum(static_times):.3f}s ({len(static_requests)} files)"
            )
