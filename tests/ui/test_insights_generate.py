...
def test_insights_generate():
    ...
    with sync_playwright() as p:
        ...
        page.on("console", lambda msg: console_errors.append(f"{msg.type}: {msg.text}") if msg.type in ['error', 'exception'] else None)
        ...
        if generate_btn:
            ...
            while time.time() - start_time < max_wait:
                ...
                error_el = page.query_selector('.alert-danger, .error-message, [class*="error"], [class*="alert"]')
                if error_el and error_el.is_visible():
                    error_text = error_el.text_content()
                    if "error" in error_text.lower() or "failed" in error_text.lower() or "失败" in error_text or "错误" in error_text:
                        error_msg = error_text
                        break
                
                # Check for success indicators
                score_el = page.query_selector('[class*="score"], [class*="result"], [class*="report"], [class*="insight"]')
                loading_el = page.query_selector('[class*="loading"], [class*="spinner"], [class*="progress"]')
