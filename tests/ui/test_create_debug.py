...
def test_create_debug():
    ...
    with sync_playwright() as p:
        ...
        def add_console_handler(pg):
            pg.on(
                ...
                lambda msg: console_messages.append(
                    {"type": msg.type, "text": msg.text, "page": "iframe"}
                ),
            )
        ...
        try:
            ...
            click_debug = [
                m for m in console_messages if "CLICK" in m["text"] or "SUBMIT" in m["text"]
            ]
            ...
            project_calls = [
                c
                for c in api_calls
                if "/api/projects" in str(c.get("url", ""))
                and ("POST" in str(c.get("method", "")) or c.get("status"))
            ]
