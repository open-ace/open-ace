...
async def test_auto_click():
    async with async_playwright() as p:
        ...
        # 记录网络请求
        requests_log = []
        page.on('request', lambda r: requests_log.append(f"{r.method} {r.url}") if '/api/' in r.url else None)
        page.on('response', lambda r: print(f"[Response] {r.status} {r.url}") if '/api/' in r.url else None)
