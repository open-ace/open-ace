...
async def test_alert_icon():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if dot_count > 0:
                ...
                results.append(
                    (
                        "Icon Check",
                        "Fail",
                        f"Found {dot_count} bi-dot icons. Should use proper icons.",
                    )
                )
            else:
            ...
            section_header = page.locator(
                '.nav-section-header:has-text("Governance"), .nav-section-header:has-text("治理")'
            )
            ...
            quota_alerts_nav = page.locator(
                '.nav-item:has-text("Quota"), .nav-item:has-text("配额")'
            )
                    ...
                    if dot_count_alerts > 0:
                        results.append(
                            (
                                "Alerts Icon Check",
                                "Fail",
                                f"Found {dot_count_alerts} bi-dot icons in alerts tab.",
                            )
                        )
                    else:
                        results.append(
                            ("Alerts Icon Check", "Pass", "No bi-dot icons in alerts tab.")
                        )
                    ...
                    if envelope_fill_count > 0:
                        results.append(
                            (
                                "Envelope Fill Icon",
                                "Pass",
                                f"Found {envelope_fill_count} bi-envelope-fill icons",
                            )
                        )
                        ...
                        results.append(
                            (
                                "Envelope Icon",
                                "Pass",
                                f"Found {envelope_count} bi-envelope icons (checking classes...)",
                            )
                        )
                    else:
                        results.append(
                            (
                                "Envelope Icon",
                                "Info",
                                f"No envelope icons found in stat cards. Icons found: {icon_classes[:10]}...",
                            )
                        )
                else:
    ...
    for step, status, detail in results:
        symbol = (
            "✓"
            if status == "Pass"
            else "✗" if status == "Fail" else "ℹ" if status in ("Info", "Skip") else "?"
        )
