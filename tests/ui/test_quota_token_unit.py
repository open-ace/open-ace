        ...
        try:
            ...
            if count > 0:
                ...
                for i in range(label_count):
                    ...
                    if (
                        "Token" in label_text
                        or "token" in label_text
                        or "token_quota" in label_text.lower()
                    ):
                        # Daily token quota
                        if "每日" in label_text or "Daily" in label_text or "daily" in label_text:
                            assert (
                                "(M)" in label_text
                            ), f"Expected '(M)' in daily token label, got: {label_text}"
                            print(f"  ✓ Daily token quota label contains (M): '{label_text}'")
                            daily_token_found = True
                        # Monthly token quota
                        elif (
                            "每月" in label_text
                            or "Monthly" in label_text
                            or "monthly" in label_text
                        ):
                            assert (
                                "(M)" in label_text
                            ), f"Expected '(M)' in monthly token label, got: {label_text}"
                ...
                for i in range(label_count):
                    ...
                    if (
                        ...
                        or "请求" in label_text
                        or "request_quota" in label_text.lower()
                    ):
                        assert (
                            "(M)" not in label_text
                        ), f"Unexpected '(M)' in request quota label: {label_text}"
