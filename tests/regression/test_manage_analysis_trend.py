...
def save_screenshot(page, name):
    ...
    path = os.path.join(SCREENSHOT_DIR, f'manage_analysis_trend_{name}.png')
...
def test_time_range_filter():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 查找时间范围选择器
            time_filter = page.locator('select, .time-range-selector, button:has-text("Day"), button:has-text("Week")')
...
def test_data_export():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            save_screenshot(page, '04_export')
            return True
        finally:
            browser.close()
...
def test_chart_interaction():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            charts = page.locator('canvas, .chart')
            if charts.count() > 0:
                # 尝试悬停在图表上
                charts.first.hover()
                page.wait_for_timeout(500)

            save_screenshot(page, '05_interaction')
            return True
        finally:
            browser.close()
...
def run_all_tests():
    ...
    results = []
    print("\n" + "=" * 60)
    print("Manage 模式 - Analysis - Trend 回归测试")
    print("=" * 60)
