...
def test_aggregator(db: Database, user_id: int, username: str, days: int = 7):
    ...
    return {"aggregation_time_ms": elapsed_ms, "records_created": records}
...
def run_performance_test():
    ...
    if original_stats["avg_ms"] > 0 and optimized_stats["avg_ms"] > 0:
        improvement = (
            (original_stats["avg_ms"] - optimized_stats["avg_ms"]) / original_stats["avg_ms"]
        ) * 100
...
""")
    ...
    return {
        ...
        "improvement_percent": (
            ((original_stats["avg_ms"] - optimized_stats["avg_ms"]) / original_stats["avg_ms"])
            * 100
            if original_stats["avg_ms"] > 0
            else 0
        ),
        "speedup": (
            original_stats["avg_ms"] / optimized_stats["avg_ms"]
            if optimized_stats["avg_ms"] > 0
            else float("inf")
        ),
    }
