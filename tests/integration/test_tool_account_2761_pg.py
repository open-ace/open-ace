...
def _insert_user(pg_db, username="testuser", email=None, tenant_id=None):
    """Insert a user and return the id."""
    if email is None:
        email = f"{username}@example.com"
    query = "INSERT INTO users (username, email, password_hash, role, tenant_id) VALUES (%s, %s, %s, %s, %s) RETURNING id"
    row = pg_db.fetch_one(query, (username, email, "hashed_pw", "user", tenant_id), commit=True)
    return row["id"]
...
def _insert_daily_message(pg_db, sender_name, user_id=None, tenant_id=None):
    """Insert a daily_messages row for testing activation."""
    row = pg_db.fetch_one(
        "INSERT INTO daily_messages (date, sender_name, user_id, tenant_id, message_source) "
        "VALUES (%s, %s, %s, %s, %s) RETURNING id",
        (datetime.now().date(), sender_name, user_id, tenant_id, "qwen"),
        commit=True,
    )
    return row["id"]
...
class TestPredeclaredAccount:
    ...
    def test_create_predeclared_account(self, pg_db):
        ...
        tenant_id = _insert_tenant(pg_db, name="tenant1")
    ...
    def test_create_manual_account_defaults_to_active(self, pg_db):
    ...
    def test_get_by_status_pending(self, pg_db):
...
class TestActivationFlow:
    ...
    def test_get_pending_for_activation(self, pg_db):
        """Batch query pending accounts by sender_names."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="eve")
        tenant_id = _insert_tenant(pg_db, name="tenant4")
    ...
    def test_activate_fails_on_version_mismatch(self, pg_db):
    ...
    def test_update_status_with_version(self, pg_db):
    ...
    def test_touch_activity(self, pg_db):
        """Update last_activity_at timestamp."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="henry")
    ...
    def test_increment_message_count(self, pg_db):
        """Increment observed_message_count."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="ivy")
...
class TestConflictDetection:
    ...
    def test_create_or_ignore_on_conflict(self, pg_db):
        """create_or_ignore returns None when account exists."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="jack")
    ...
    def test_conflict_status_enum_values(self, pg_db):
...
class TestStaleDetection:
    ...
    def test_get_stale_mappings(self, pg_db):
        """Get accounts with no activity for specified days."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="kate")
        tenant_id = _insert_tenant(pg_db, name="tenant5")
    ...
    def test_get_stale_mappings_no_activity(self, pg_db):
        """Accounts with no activity_at are considered stale."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="leo")
...
class TestConcurrentActivation:
    ...
    def test_concurrent_activation_version_check(self, pg_db):
        """Verify optimistic lock prevents concurrent activation."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="mike")
...
class TestMigrationScenarios:
    ...
    def test_migrate_discovered_account(self, pg_db):
        """Migrate existing account to discovered status."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="nancy")
        tenant_id = _insert_tenant(pg_db, name="tenant6")
    ...
    def test_migrate_predeclared_without_data(self, pg_db):
        """Predeclared account without data remains pending."""
        repo = UserToolAccountRepository(db=pg_db)
        user_id = _insert_user(pg_db, username="oscar")
