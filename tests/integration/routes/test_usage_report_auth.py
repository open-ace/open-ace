"""
Integration tests for Usage Report Security Hardening (Issue #1891).

Tests cover authentication, binding validation, rate limiting, and audit
logging over both production ingress paths (/api/remote/usage-report and
/api/remote/agent/message).

Uses RemoteAgentManager directly with a temporary SQLite database.

Pure input-validation tests live in tests/unit/test_usage_report_validation.py.
"""
...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1891), pytest.mark.security]
