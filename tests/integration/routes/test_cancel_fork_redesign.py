"""Route tests for the cancel/fork redesign (Issue #886).

Covers the API surface:
- POST .../milestones/<id>/cancel (cancel milestone with user feedback)
- POST .../milestones/<id>/fork (fork milestone into an independent workflow)
- GET .../forks (fork listing)
- POST .../resume-with-feedback (resume a waiting workflow with feedback)

Migrated from tests/issues/886/test_cancel_fork_redesign.py (route classes).
The repository layer behind the routes is mocked (``auto_repo``); a real
temporary SQLite database backs ``create_app``'s bootstrap and the auth user
lookup.
"""
