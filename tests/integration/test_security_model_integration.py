...
def _patch_db_compat():
    ...
    ``%s`` placeholders and psycopg2-style connections against a SQLite DB. Patch
    every module that holds its own binding so the test is independent of import
    order (it must pass whether run alone or as part of the full suite).
    """
    with (
