...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1823)]


@pytest.fixture
def temp_db(tmp_path, monkeypatch):
    """An isolated SQLite runtime DB built from the authoritative schema.

    The manager's background pollers (heartbeat/retention greenlets and the
    pending-revoke daemon thread) are disabled for the suite: each test
    constructs several managers and those loops would otherwise leak across
    tests. Both ``is_postgresql`` flags (manager module and database module —
    the latter drives ``_param()``) are forced off so an ambient Postgres
    config cannot leak ``%s`` placeholders into the sqlite SQL.
    """
    ...
    db_path = str(tmp_path / "runtime_1823.db")
...
class TestBufferOutputBatched:
    ...
    def test_buffer_output_flushes_on_timeout(self, temp_db):
        ...
        # Age the accumulator deterministically past the flush interval
        # (equivalent to waiting out the 50ms timer, without the sleep).
        acc.last_flush = time.time() - (manager.OUTPUT_BATCH_INTERVAL_MS / 1000 + 0.05)
...
class TestIsSessionEndedLogRateLimiting:
    ...
    def test_is_session_ended_returns_true_for_ended_session(self, temp_db):
        ...
        with manager.db.connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO agent_sessions (session_id, status, tool_name) VALUES (?, ?, ?)",
                ("test-session", "completed", "qwen"),
            )
            conn.commit()
    ...
    def test_log_rate_limiting_on_db_error(self, temp_db, caplog):
        """DB-failure logging fires once per (session, minute) window."""
        manager = RemoteAgentManager(db_path=temp_db)
        minute = int(time.time()) // 60

        with caplog.at_level(logging.WARNING, logger="app.modules.workspace.remote_agent_manager"):
            manager._log_session_ended_db_failure_cached("test-session", minute)
            # Same window again: suppressed by the cache.
            manager._log_session_ended_db_failure_cached("test-session", minute)
            # New window: logs again.
            manager._log_session_ended_db_failure_cached("test-session", minute + 1)

        # The cache holds exactly the two distinct (session, minute) keys.
        assert set(manager._log_rate_limit_cache) == {
            f"test-session_{minute}",
            f"test-session_{minute + 1}",
        }
        # One warning per window — the repeat call within the window is muted.
        warnings = [
            r for r in caplog.records if "DB query failed in is_session_ended" in r.getMessage()
        ]
        assert len(warnings) == 2
        assert all("test-ses" in w.getMessage() for w in warnings)
