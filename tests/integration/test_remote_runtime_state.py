...
pytestmark = [pytest.mark.regression, pytest.mark.issue(1782)]
...
def runtime_db(tmp_path, monkeypatch):
    ...
    # _param()/Database.connection() consult the database module's own flag;
    # patch it too so an ambient Postgres config (HOME ~/.open-ace/config.json)
    # cannot leak %s placeholders into the sqlite runtime DB.
    ...
    # The pending-revoke cleaner spawns a real daemon thread per manager
    # instance; it polls on a 60s loop and is irrelevant to replay semantics.
