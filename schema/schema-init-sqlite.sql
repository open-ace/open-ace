...
CREATE INDEX idx_usage_summary_host_name_valid ON usage_summary (host_name) WHERE ((host_name IS NOT NULL) AND ((host_name) <> '') AND ((host_name) !~~ '<%>') AND ((length((host_name)) >= 1) AND (length((host_name)) <= 253)));
