...
def upgrade():
    ...
    with op.batch_alter_table('content_filter_rules', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('approval_status', sa.String(20), nullable=False, server_default='approved'))
    ...
    op.create_index('idx_content_filter_rules_approval_status', 'content_filter_rules', ['approval_status'])
    op.create_index('idx_content_filter_rules_priority', 'content_filter_rules', ['priority'])
    
    # 新增约束
    with op.batch_alter_table('content_filter_rules', schema=None) as batch_op:
        batch_op.create_check_constraint('chk_system_rule_immutable', "source != 'system' OR is_test = FALSE")
        batch_op.create_check_constraint('chk_approval_status_valid', "approval_status IN ('pending', 'approved', 'rejected')")
    ...
    op.create_table(
        ...
        sa.Column('rule_id', sa.Integer(), sa.ForeignKey('content_filter_rules.id', ondelete='CASCADE'), unique=True, nullable=False),
        sa.Column('trigger_count', sa.BigInteger(), server_default='0'),
        sa.Column('last_triggered_at', sa.DateTime(), nullable=True),
    )
    op.create_index('idx_filter_rule_trigger_stats_rule_id', 'filter_rule_trigger_stats', ['rule_id'])
