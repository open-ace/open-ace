...
def upgrade() -> None:
    ...
    # For PostgreSQL, drop the unique constraint first to avoid name conflict
    if is_postgresql:
        op.execute("ALTER TABLE quota_usage DROP CONSTRAINT IF EXISTS uq_quota_usage_user_date_period")
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('user_id', 'date', 'period', name='uq_quota_usage_user_date_period_new'),
    )
