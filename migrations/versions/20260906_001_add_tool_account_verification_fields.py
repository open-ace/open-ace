...
def upgrade() -> None:
    ...
    op.add_column(
        "user_tool_accounts", sa.Column("verification_status", sa.String(50), nullable=True)
    )

    # Add verification_result field
    op.add_column("user_tool_accounts", sa.Column("verification_result", sa.Text(), nullable=True))

    # Add verified_at field
    op.add_column("user_tool_accounts", sa.Column("verified_at", sa.DateTime(), nullable=True))
