...
def upgrade():
    ...
    # 创建索引（无论表是刚创建还是已存在）
    trigger_log_indexes = {idx["name"] for idx in inspector.get_indexes("filter_rule_trigger_log")}
    ...
    # 创建索引（无论表是刚创建还是已存在）
    rule_versions_indexes = {idx["name"] for idx in inspector.get_indexes("filter_rule_versions")}
