...
| `alerts.dingtalk_webhook_secret` | 钉钉自定义机器人“加签”密钥（全局，所有用户共用）。留空时按普通 webhook 发送；配置后发送前自动附加 `timestamp` / `sign`。也可在「告警偏好」里按用户设置各自的密钥（写入 webhook URL 的 `openace_dingtalk_secret=<secret>`，Fernet 加密存储），其优先级高于此全局值。 | `""` |
