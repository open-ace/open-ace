...
| 测试阶段 | 测试内容 | 测试文件 | 验证标准 |
|----------|----------|----------|----------|
| preparation | git clone, git branch | tests/unit/test_git_network_retry.py | 命令执行成功 |
| execution | git commit, git push | tests/unit/test_github_ops.py | 命令执行成功 |
| pr | gh pr create, gh pr view, gh pr merge | tests/unit/test_github_ops.py | 命令执行成功 |
| issue | gh issue view, gh issue create | tests/unit/test_github_ops.py | 命令执行成功 |
| **完整工作流** | autonomous 端到端测试 | tests/autonomous/（目录级回归） | **100%通过** |
...
当前 Dockerfile 在同一层内下载并安装 gh deb 包，构建后不在 `/tmp` 留存缓存（无跨构建缓存过期问题）。
以下为 **规划中的缓存策略**（若未来引入跨构建缓存则按此执行）：
...
需要手动清理时，可保存以下脚本为 `scripts/clean-gh-cache.sh` 使用（仓库不自带该脚本）：
...
### 相关文档
- 版本兼容性管理文档：docs/GH_CLI_VERSION_COMPATIBILITY.md
- sudoers审计报告：docs/dev-notes/sudoers-audit-report.md
- 镜像源验证报告：docs/dev-notes/mirror-source-validation-report.md
```
