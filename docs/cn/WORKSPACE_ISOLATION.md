...
- `webui_callback_url` **必填**:它是沙箱唯一可以访问的地址(Open ACE API 与 LLM 代理),取自服务端配置,
  而不是请求。
- `confinement_egress_allow` 追加 `host:port`;其余一律拒绝,并记入
...
2. **WebUI 镜像**:用 `scripts/docker/webui-sandbox.Dockerfile` 构建,并在 **root 策略文件**
   ...
   ```
3. **`config.json`**:与 §3.3 相同的配置块,改为 `"os_user_confinement": "runsc"`。
...
1. **运行时:** 把 `containerd-shim-kata-v2` 放在 dockerd 的 PATH 上(如 `/usr/local/bin`),无需修改
   `daemon.json`,也无需重启 Docker。嵌套虚拟化下,在 `/etc/kata-containers/configuration.toml` 中调大
   `dial_timeout`(例如 180)。
2. **root 策略文件:** 在同一个 `container` 段里加入 Kata 运行时:
   ```json
   "runtimes": {"runsc": "runsc-openace", "kata": "io.containerd.kata.v2"}
   ```
3. **`config.json`**:与 §3.3 相同的配置块,改为 `"os_user_confinement": "kata"`。
...
参考:[能力契约 §6](WORKSPACE_ISOLATION_CAPABILITIES.md) 与 [SANDBOX_BACKENDS §8](SANDBOX_BACKENDS.md#8-交互工作区sandboxed-隔离等级)。
...
- `reasons[].message`:配置了某种方式但宿主无法提供时,部署**绝不会静默以更弱的隔离运行**,而是报告
  `launch_path_degraded`,并在括号里给出确切原因。
...
| `confinement_runtime_missing` | 注册 gVisor 运行时 / 把 Kata shim 放到 dockerd 的 PATH 上 |
...
| `sandbox_backend_unconfigured`、`webui_image_*`、`sandbox_proxy_unreachable` | OpenSandbox pod 配置问题,见[能力契约 §3.4](WORKSPACE_ISOLATION_CAPABILITIES.md#34-sandboxed-探测与运行期原因码3378) |
...
工作区启动都会被拒绝并返回结构化错误,而不是以更弱的隔离启动。多用户安装会固定为 `os_user`。不设置时,
要求会跟随部署当前验证到的等级,启动路径退化时它可能静默下降。请求可以抬高要求
...
- **忘了 `webui_callback_url`。** 所有受约束与 sandboxed 的方式没有它都拒绝运行。
