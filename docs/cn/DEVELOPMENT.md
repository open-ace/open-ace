...
5. 发布 GitHub Release 并附带 `dist/open-ace-X.Y.Z.tar.gz`；
   `.github/workflows/release.yml` 随后构建 sdist/wheel 并附到 Release，再通过 Trusted Publishing 发布到 PyPI（由仓库变量 `PYPI_PUBLISH=true` 开启，无需 API token）；`docker-publish.yml` 推送 GHCR 镜像
