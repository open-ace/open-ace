...
Open ACE 同时支持 SQLite（单机）和 PostgreSQL（生产环境）。模式包含 103 张表 + 1 个物化视图（完整列表见 schema-postgres.sql，以下为常用表）。
...
命名约定请参阅 [DATABASE_CONVENTIONS.md](DATABASE_CONVENTIONS.md)。`app/repositories/database.py` 中的 `adapt_sql()` 函数自动处理占位符转换（`?` → `%s`）。
