...
3. 新出现的失败检查进入 CI 修复，新出现的 pending 检查继续等待；
4. `blocked`、`behind`、`unstable`、`has_hooks`、`draft`，以及 repository rules、required checks、branch protection 等明确策略拒绝，都按仓库策略等待处理，不启动冲突 Agent；
5. 只有 `mergeable_state=dirty`、明确的 conflict 错误，或 GitHub 只返回 `mergeable=false` 且没有更具体状态时，才进入冲突解析；
6. 未知权限、API 或基础设施错误保留原始错误，不能改写成“Merge conflict resolution failed”。
...
- PR 分支落后主分支；
- 分支保护返回 `repository rule violations`，同时覆盖 pending check、`blocked`、`behind`、`unstable`、`has_hooks` 和 `draft`；
- `mergeable_state=dirty` 的真实冲突，以及未知权限/API 错误不会被误判为冲突；
- 远端 head 与本地 head 不同；
- 本地存在未推送合法提交；
- worktree 被冲突解析临时移除；
...
| 分支保护触发冲突解析 | 合并拒绝后是否刷新 checks 和 REST merge state；策略状态必须等待，只有 `dirty`/明确冲突才进入 resolver |
...
- 仓库策略等待不会消耗 CI 修复次数，但仍可能需要用户完成审批、解除 draft 或调整仓库规则。
