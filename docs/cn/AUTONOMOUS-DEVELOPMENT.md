...
1. 最终合并前先用受控路径检查并同步落后于 `main` 的 PR 分支；同步会触发新 CI，本轮直接返回等待，不消耗 CI repair attempt；
2. 合并调用被拒绝后，重新查询检查结果和 GitHub REST `mergeable` / `mergeable_state`，避免使用调用前的过期状态；
3. 新出现的失败检查进入 CI 修复；pending 检查属于已确认的瞬时状态，继续等待；
4. 没有 pending 检查时，先判定真实冲突：`mergeable_state=dirty`、明确的 conflict 错误，或 GitHub 只返回 `mergeable=false` 且没有更具体状态时进入 resolver；权威的 `dirty` 优先于同一次响应里的通用 repository-rule 文本；
5. 没有失败/pending/冲突证据，但错误明确表示 repository rules、required checks、review、draft 或 branch protection 时，工作流进入可人工恢复的 `paused`；用户满足仓库要求后再恢复；
6. `blocked`、`behind`、`unstable`、`has_hooks`、`draft` 等 merge state 只用于补充诊断，不能单独吞掉权限、API 或基础设施错误；未知错误保留原始原因并失败可见。
...
| 验收 verifier 基础设施失败 | 运行器失败、验收输出不可解析 | 同阶段自动重试，最多 3 次（确定性解析失败最多 2 次）；耗尽后暂停等待人工 |
...
- PR 分支落后主分支；
- 分支保护返回 `repository rule violations`：pending check 继续等待，无 pending 的明确策略拒绝进入可人工恢复的暂停；
- `dirty + repository rule` 仍进入 resolver；`blocked + Resource not accessible` 保留权限错误而不无限等待；
- 远端 head 与本地 head 不同；
- 本地存在未推送合法提交；
- worktree 被冲突解析临时移除；
...
| 分支保护触发冲突解析或无限等待 | 合并拒绝后是否刷新 checks 和 REST merge state；pending 才继续等待，`dirty` 优先解析，无 pending 的明确策略拒绝应暂停 |
