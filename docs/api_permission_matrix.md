# API Permission Matrix

Issue #2276: Permission requirements for all admin endpoints.

## Overview

This document lists all API endpoints that require elevated permissions (admin, platform_admin, or tenant_admin).

## Permission Model

| Role | Description | Platform Admin APIs | Tenant APIs |
|------|-------------|-------------------|------------|
| `admin` | Legacy admin role (backward compatible) | ✅ Full access | ✅ Full access |
| `platform_admin` | Platform admin (recommended) | ✅ Full access | ✅ Full access |
| `tenant_admin` | Tenant admin | ❌ No access | ✅ Own tenant only |
| `user` | Regular user | ❌ No access | ❌ No access |

## Platform Admin Required Endpoints

These endpoints require `platform_admin` or `admin` role.

| Method | Path | Function | File | Line |
|--------|-----|----------|------|------|
| POST | `` | `create_tenant` | tenant.py | 85 |
| GET | `/<int:tenant_id>` | `get_tenant` | tenant.py | 43 |
| PUT | `/<int:tenant_id>` | `update_tenant` | tenant.py | 101 |
| DELETE | `/<int:tenant_id>` | `delete_tenant` | tenant.py | 372 |
| POST | `/<int:tenant_id>/activate` | `activate_tenant` | tenant.py | 339 |
| PUT | `/<int:tenant_id>/quota` | `update_tenant_quota` | tenant.py | 208 |
| PUT | `/<int:tenant_id>/settings` | `update_tenant_settings` | tenant.py | 261 |
| GET | `/<int:tenant_id>/usage` | `get_tenant_usage` | tenant.py | 400 |
| GET | `/slug/<slug>` | `get_tenant_by_slug` | tenant.py | 69 |

## Admin Required Endpoints

These endpoints require `admin`, `platform_admin`, or `tenant_admin` role.

## Same Tenant or Platform Admin Endpoints

These endpoints allow tenant_admin to access their own tenant, or platform_admin to access any tenant.

| Method | Path | Function | File | Line |
|--------|-----|----------|------|------|
| POST | `/<int:tenant_id>/check-quota` | `check_tenant_quota` | tenant.py | 450 |
| GET | `/<int:tenant_id>/stats` | `get_tenant_stats` | tenant.py | 429 |
| POST | `/<int:tenant_id>/suspend` | `suspend_tenant` | tenant.py | 296 |
| GET | `/plans` | `get_plan_quotas` | tenant.py | 466 |

## Summary

- Total platform_admin_required endpoints: 9
- Total admin_required endpoints: 0
- Total same_tenant_or_platform_admin endpoints: 4

**Note**: Issue #2276 ensures backward compatibility - `admin` role can access all `platform_admin_required` endpoints.

---

Generated by: `scripts/generate_permission_matrix.py`
Last updated: 2026-08-05
