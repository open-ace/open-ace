...
class DailyStatsRepository:
    ...
    def refresh_hourly_stats(self, date: str | None = None) -> bool:
        ...
        # Issue #2333: Use SchedulerExecutionGuard for fail-closed lock management
        # Uses separate lock key from daily_stats_refresh to allow parallel execution
        guard = SchedulerExecutionGuard(
        ...
        try:
            with guard:
                # Job body only runs if lock acquired
                self._execute_refresh_hourly_stats(date, start_time)
                return True
            ...
            duration_ms = (time.time() - start_time) * 1000
            logger.info(
                f"refresh_hourly_stats skipped for {date or 'all dates'}: {e} (took {duration_ms:.2f}ms)"
            )
            return False

        except Exception as e:
            duration_ms = (time.time() - start_time) * 1000
            logger.error(f"Failed to refresh hourly stats: {e} (took {duration_ms:.2f}ms)")
            return False

    def _execute_refresh_hourly_stats(self, date: str | None, start_time: float) -> None:
        """Execute the actual refresh_hourly_stats logic (called within guard context)."""
        now = datetime.now(timezone.utc).replace(tzinfo=None)
        ...
        duration_ms = (time.time() - start_time) * 1000
        logger.info(f"Hourly stats refreshed for {date or 'all dates'} (took {duration_ms:.2f}ms)")
