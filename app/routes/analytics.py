...
def api_usage_report():
    ...
    if not is_admin:
        return jsonify(session_or_error), 403 if 'Admin' in session_or_error.get('error', '') else 401
...
def api_export_analytics():
    ...
    if format_type == 'csv':
        ...
        if report.trends:
            ...
            writer.writerow(['Metric', 'Direction', 'Change %', 'Current', 'Previous', 'Confidence'])
        ...
        if report.anomalies:
            ...
            writer.writerow(['Type', 'Metric', 'Date', 'Expected', 'Actual', 'Deviation %', 'Severity'])
