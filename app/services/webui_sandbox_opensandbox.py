...
``OpenSandboxWebuiLauncher`` owns one qwen-code-webui process per user inside an
...
The backend-neutral pieces (heartbeats, token mint, the local port proxy,
the launch result/error types) live in ``app.services.webui_sandbox`` (#3431).
...
from app.services import webui_sandbox as _shared
...
class OpenSandboxWebuiLauncher:
        ...
        try:
            body = sandbox_policy_mod.build_create_request(
                ...
                extra_metadata={
                    ...
                    WEBUI_METADATA_GENERATION: _shared.current_process_generation(),
...
def reconcile_webui_orphans(
    *,
    ...
    if _shared.write_webui_heartbeat(state_root_override) is None:
        logger.warning(
        ...
        return []
    try:
        peers = _shared.fresh_peer_heartbeats(state_root_override)
    except OSError as exc:
    ...
    if peers:
        ...
        return []
    destroyed: list[str] = []
    mine = _shared.current_process_generation()
    query = {
...
def _delete_orphan(
    ...
    if not confirmed:
        return False
    record = OpenSandboxWebuiLauncher._safe_restore_record_path(sandbox_id, state_root_override)
