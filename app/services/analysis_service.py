...
class AnalysisService:
    ...
    def get_batch_analysis(
        ...
        sorted_hours = (
            sorted(hourly_totals.items(), key=lambda x: x[1], reverse=True) if hourly_totals else []
        )
        ...
        peak_usage = {
            ...
            "average_daily": (
                sum(daily_totals_for_peak.values()) / len(daily_totals_for_peak)
                if daily_totals_for_peak
                else 0
            ),
        }
    ...
    def get_daily_hourly_usage(
        ...
        # Process daily data directly from aggregated query
        daily_result = []
        for d in results.get("daily_data", []):
            daily_result.append(
                {
                    "date": d.get("date"),
                    "tokens": d.get("total_tokens", 0),
                    "requests": d.get("message_count", 0),
                }
            )

        # Process hourly data
        hourly_result = []
        for h in results.get("hourly_data", []):
    ...
    def get_conversation_stats(
        ...
        # with get_batch_analysis / get_key_metrics).
        if not start_date:
            start_date = get_days_ago(30)
        if not end_date:
            end_date = get_today()
    ...
    def get_recommendations(self, host_name: Optional[str] = None) -> list[dict]:
        ...
        if not recommendations:
            recommendations.append({"type": "success", "message": "Usage patterns look healthy"})
    ...
    def get_anomaly_trend(
        ...
        for anomaly in anomalies:
            ...
            if date not in trend_by_date:
                trend_by_date[date] = {"date": date, "count": 0, "spikes": 0, "drops": 0}
