...
class CollaborationManager:
    ...
    def create_team(
        ...
        return Team(
            ...
            owner_id=owner_id,
            members=[
                TeamMember(user_id=owner_id, username="", role=TeamRole.OWNER.value, joined_at=now)
            ],
    ...
    def add_team_member(
        self, team_id: str, user_id: int, username: str = "", role: str = TeamRole.MEMBER.value
    ) -> bool:
        """
    ...
    def revoke_share(self, share_id: str, user_id: Optional[int] = None) -> bool:
        ...
        if user_id:
            cursor.execute(
                "DELETE FROM shared_sessions WHERE share_id = ? AND shared_by = ?",
                (share_id, user_id),
            )
        else:
    ...
    def delete_annotation(self, annotation_id: str, user_id: Optional[int] = None) -> bool:
        ...
        if user_id:
            cursor.execute(
                "DELETE FROM annotations WHERE annotation_id = ? AND user_id = ?",
