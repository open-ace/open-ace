...
**현상:** 시작 시 오류: `Error: Address already in use (0.0.0.0:5000)`

**원인:**
1. 다른 서비스가 5000 포트 사용 중
2. 이전 Open ACE 프로세스가 완전히 종료되지 않음

**해결 방법:**
1. 포트 사용 확인: `lsof -i :5000` 또는 `netstat -tlnp | grep 5000`
...
**현상:** 새 세션 생성 시 "Maximum instances (20) reached" 표시
