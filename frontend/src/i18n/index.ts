...
export const translations: Record<Language, Translations> = {
  en: {
    ...
    enableSSO: 'Enable Global SSO',
    ssoEnabledDesc: 'This switch controls SSO login for all tenants on the login page',
    ssoGlobalHint: 'This is a global setting that affects all tenants. Only effective for the default tenant.',
    ssoReadOnlyHint: 'SSO is controlled by the default tenant settings.',
  ...
  zh: {
    ...
    enableSSO: '全局启用 SSO',
    ssoEnabledDesc: '此开关控制登录页面上所有租户的 SSO 登录',
    ssoGlobalHint: '这是一个全局设置，影响所有租户。仅在默认租户下有效。',
    ssoReadOnlyHint: 'SSO 由默认租户设置控制。',
  ...
  ja: {
    ...
    enableSSO: 'SSOをグローバルに有効化',
    ssoEnabledDesc: 'このスイッチはログインページですべてのテナントのSSOログインを制御します',
    ssoGlobalHint: 'これはすべてのテナントに影響するグローバル設定です。デフォルトテナントでのみ有効です。',
    ssoReadOnlyHint: 'SSOはデフォルトテナントの設定によって制御されています。',
  ...
  ko: {
    ...
    enableSSO: 'SSO 전역 활성화',
    ssoEnabledDesc: '이 스위치는 로그인 페이지에서 모든 테넌트의 SSO 로그인을 제어합니다',
    ssoGlobalHint: '이것은 모든 테넌트에 영향을 미치는 전역 설정입니다. 기본 테넌트에서만 유효합니다.',
    ssoReadOnlyHint: 'SSO는 기본 테넌트 설정에 의해 제어됩니다.',
