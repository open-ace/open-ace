...
if [ "$WORKSPACE_MULTI_USER_MODE" = "true" ] || [ "$CONFIG_MULTI_USER" = "true" ]; then
    ...
    if mkdir -p "$_chown_conf_dir" 2>/dev/null; then
        {
            echo "# Generated by docker-entrypoint.sh at boot — do not edit while running."
            echo "# Consumed by scripts/openace-chown.sh (ALLOWED_PREFIXES override)."
        ...
        unset _first _wb _wb_raw _wb_list
    else
        echo "  WARNING: could not create $_chown_conf_dir — openace-chown stays on built-in prefixes (/workspace, /home)"
    fi
    unset _chown_conf_dir _chown_conf
...
try:
    ...
    for path, system_account, username in cur.fetchall():
        if not path:
            continue
        if not any(path == b or path.startswith(b + '/') for b in bases):
            continue
        if not os.path.isdir(path):
            continue
        owner = system_account or username
        if not owner:
            continue
        try:
            ...
            if group != GLOBAL_GROUP and not group.startswith(GLOBAL_GROUP + '-'):
                # Not shared-group-owned: either already reclaimed by the
                # app-side revoke or never group-shared — not ours to touch.
                continue
            if not reclaim_revoked(path, owner):
                failures += 1
        except Exception as e:  # noqa: BLE001 - one bad row must not abort the rest
            failures += 1
