...
"
    ...
    "
        ...
        # 注意：if/fi 逻辑必须放在 heredoc 之外，否则 shell 代码会被逐字写入 sudoers 文件，
        # 导致 visudo 校验失败并删除 sudoers（工作区 webui 无法启动）
        WEBUI_LAUNCH_WRAPPER="/usr/local/bin/openace-webui-launch"
        if [ -x "$WEBUI_LAUNCH_WRAPPER" ]; then
            # webui_manager 以 `wrapper <KEY=VALUE>... <WEBUI_PATH> --port ...` 格式调用，
            # 前置 `*` 用于匹配内联的 KEY=VALUE 环境变量参数（见 tests/issues/2298/test_webui_env_isolation.py）
            WEBUI_SUDO_RULE="open-ace ALL=(ALL) NOPASSWD: ${WEBUI_LAUNCH_WRAPPER} * \"${WEBUI_PATH}\" *
openace ALL=(ALL) NOPASSWD: ${WEBUI_LAUNCH_WRAPPER} * \"${WEBUI_PATH}\" *"
        else
            WEBUI_SUDO_RULE="open-ace ALL=(ALL) NOPASSWD: ${WEBUI_PATH} *
...
# ============================================================================
# WebUI 启动规则：允许以任意用户运行，Python 层验证目标用户是否在数据库映射中
# 规则文本由 heredoc 之外的 WEBUI_SUDO_RULE 变量提供（见上方 shell 逻辑）
${WEBUI_SUDO_RULE}
