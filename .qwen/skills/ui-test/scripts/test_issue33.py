        ...
        try:
            ...
            if conversations_elem:
                ...
                if conversations_text and conversations_text != "-":
                    results.append(
                        ("Conversation Stats API", True, f"conversations: {conversations_text}")
                    )
                    ...
                    if conversations_text and conversations_text != "-":
                        ...
                        print(
                            f"   ✓ Conversation Stats 加载成功: {conversations_text} conversations"
                        )
                    else:
            ...
            if table_container:
                ...
                table_html = await page.evaluate(
                    '() => document.getElementById("conversation-history-table").innerHTML'
                )
                if "tabulator" in table_html:
                    ...
                    try:
                        ...
                        results.append(
                            ("切换到 Conversation History", True, "表格存在但可能无数据")
                        )
            ...
            timeline_btn = await page.query_selector(
                ".tabulator-row:first-child .btn-outline-primary"
            )
            if timeline_btn:
            ...
            latency_btn = await page.query_selector(
                ".tabulator-row:first-child .btn-outline-success"
            )
            if latency_btn:
            ...
            # 点击第一行的非按钮单元格来触发详情
            first_row_cell = await page.query_selector(
                "#conversation-history-table .tabulator-row:first-child .tabulator-cell:not(:has(button))"
            )
            if first_row_cell:
                ...
                try:
                    ...
                    # 截图
                    screenshot_path = (
                        f"{SCREENSHOT_DIR}/issue33_conversation_detail_{timestamp}.png"
                    )
                    await page.screenshot(path=screenshot_path)
                    print(f"   截图保存: {screenshot_path}")
