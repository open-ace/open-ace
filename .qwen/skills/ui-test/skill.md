...
### 1. 先用 HEADLESS 模式测试
- 设置 `HEADLESS = True`，浏览器在后台运行
- 执行测试用例，验证功能是否正常
- 如果测试失败，修复问题后重新测试
- 重复直到测试通过

### 2. 询问用户是否需要可视化验证
测试通过后，使用 `ask_user_question` 工具询问用户：

```
问题: "测试已通过，是否需要用非 HEADLESS 模式查看测试效果？"
选项:
- "是，打开浏览器窗口查看" - 使用 HEADLESS = False 重新运行测试
- "否，测试已完成" - 结束测试流程
```

### 3. 根据用户选择执行
- **如果用户选择"是"**：设置 `HEADLESS = False`，重新运行测试，用户可以观察浏览器窗口中的测试过程
- **如果用户选择"否"**：测试流程结束
