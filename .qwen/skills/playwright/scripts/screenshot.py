...
SCREENSHOT_TARGETS = {
    "full": {"description": "Full page screenshot", "selector": None, "full_page": True},
    "dashboard": {
        "description": "Dashboard section",
        "selector": "#dashboard-section",
        "full_page": False,
    },
    "messages": {"description": "Messages section", "selector": ".messages", "full_page": False},
    "analysis": {
    ...
    with sync_playwright() as p:
        ...
        if needs_messages:
            try:
                ...
                messages_selectors = ["#nav-messages", 'a:has-text("Messages")', "text=Messages"]
                clicked = False
        ...
        needs_analysis = any(
            t
            in ["analysis", "datepicker", "heatmap", "metrics", "tokens", "peak", "session-history"]
            for t in targets
        )
        if needs_analysis:
            try:
                # Try multiple selectors for Analysis link
                analysis_selectors = ["#nav-analysis", 'a:has-text("Analysis")', "text=Analysis"]
                clicked = False
