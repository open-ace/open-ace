...
configure_sudoers() {
    ...
    # Get fetch_qwen.py path
    local fetch_qwen_path="$install_dir/scripts/fetch_qwen.py"
    if [ ! -f "$fetch_qwen_path" ]; then
        print_warning "fetch_qwen.py not found at: $fetch_qwen_path"
        fetch_qwen_path=""  # Will be skipped in sudoers
    fi
...
# Allow $run_user to run fetch_qwen.py as root for multi-user data collection
$run_user ALL=(root) NOPASSWD: /usr/bin/python3 $fetch_qwen_path *"
    fi
    if [ -f "$fetch_claude_path" ]; then
        fetch_sudoers="$fetch_sudoers
# Allow $run_user to run fetch_claude.py as root for multi-user data collection
$run_user ALL=(root) NOPASSWD: /usr/bin/python3 $fetch_claude_path *"
    fi
    if [ -f "$fetch_openclaw_path" ]; then
        fetch_sudoers="$fetch_sudoers
...
# Allow $run_user to run fetch scripts as root for multi-user data collection
# This is needed to read other users' .qwen/.claude/.openclaw directories for workspace stats
# Scripts use --config parameter to get database config from service account's config.json
$fetch_sudoers
...
"
    ...
    if [ -f "$sudoers_file" ]; then
        ...
        local has_secure_path=false

        # Check for either /usr/bin or /usr/local/bin qwen-code-webui
        if grep -q "$webui_path" "$sudoers_file" 2>/dev/null || \
           grep -q "/usr/local/bin/qwen-code-webui" "$sudoers_file" 2>/dev/null; then
            has_webui=true
        fi
        ...
        # Check for secure_path configuration
        if grep -q "secure_path.*usr/local/bin" "$sudoers_file" 2>/dev/null; then
            has_secure_path=true
        fi

        # If all exist, no need to update
        if [ "$has_webui" = true ] && [ "$has_fetch" = true ] && [ "$has_secure_path" = true ]; then
            print_success "Sudoers rules already exist with correct paths and secure_path"
            return 0
        fi

        # Need to update - missing something
        if [ "$has_secure_path" = false ]; then
            print_info "Sudoers file missing secure_path configuration, adding..."
...
install_local() {
    ...
    if [ "$WORKSPACE_MULTI_USER_MODE" = "true" ]; then
        ...
        stop_webui_systemd_service
        configure_sudoers "$DEPLOY_USER" "$target_path"
        if [ $? -ne 0 ]; then
