...
- Server-side isolation floor for workspace launches (Issue #3374): `GET /api/workspace/user-url` enforces `workspace.required_isolation_level` from config.json (default: `os_user` in multi-user mode, `none` in single-user mode); the `required_isolation` query parameter can only raise, never lower, that floor, and empty values count as absent. In multi-user mode this makes the default (parameterless) path fail closed: users without an explicit `system_account` mapping get a structured 400 (`identity_mapping_missing`) instead of a silently convention-derived launch — login no longer auto-backfills `system_account = username` in multi-user mode, cached instances whose launch account no longer matches the mapping are restarted under the current mapping, and the login-time background pre-start goes through the same gate (it never launches what the gate would reject). Rejections carry both reason namespaces (deployment-level + request-level). Single-user deployments are unaffected.
