...
### Added
- Versioned capability contract for local interactive workspace multi-user isolation (Issue #3374): `GET /api/workspace/isolation-capabilities` reports what this deployment actually enforces (`local_workspace_multi_user`, `backend`, `isolation_level`, enforced/unsupported dimensions, per-entry-point coverage, `policy_revision`), derived from live runtime state instead of README claims. `GET /api/workspace/user-url` now accepts `required_isolation=none|os_user|sandboxed`: when an explicit requirement cannot be met (capability gap, missing `system_account` mapping, or a launch path that would silently use the shared service account), the launch is rejected with a structured 400 (`error_code` + the effective contract) instead of silently degrading; without the parameter existing behavior is unchanged except for a new `isolation` field echoing the effective policy. Interactive workspaces report at most `os_user` (per-user OS accounts on a shared kernel — never advertised as strong runtime isolation); see `docs/workspace-isolation-capabilities.md` for semantics, reason-code reference, deployment requirements, and the audited list of remaining entry-point gaps split into follow-ups.
